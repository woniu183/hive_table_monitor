...
object CollectDataMarketHiveTableSize {
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("CollectDataMarketHiveTableSize4day")
    ...
    val remote_hdfs_url = properties.getProperty("remote_hdfs_url")
    ...
    try {
      ...
      while (resultSet.next()) {
        try {
          ...
          val  fileSystem=RemoteHdfsUtil.getFileSystem(remote_hdfs_url)


           val jishi_location= LOCATION.replace(begin_with,remote_hdfs_url)

          if (fileSystem.exists(new Path(jishi_location))) {

            //调用getFileSize方法 得到文件的大小FILESIZE
          //  val FILESIZE = HdfsUtil.getFileSize(fileSystem, LOCATION).toString()
            val FILESIZE =RemoteHdfsUtil.getFileSize(fileSystem,jishi_location).toString()



            //将字段写入到mysql中
            val result = statement2.executeUpdate("INSERT into t_jishi_hive_tableinfo values('" + date + "','" + DB + "','" + TABLE_NAME + "','" + SD_ID + "','" + TBL_TYPE + "','" + CREATE_TIME + "','" + LOCATION + "','" + SLIB + "','" + FILESIZE + "','" + WRITE_COUNT + "','" + READ_COUNT + "','" + src + "')" +
      ...
      val db_df = sqlContext.read.format("jdbc").options(
        Map("url" -> dbc_2,
          "dbtable" -> "t_jishi_hive_tableinfo")).load()
     ...
     result_df5.write.mode(SaveMode.Append).jdbc(jdbc_url, "t_jishi_hive_resultinfo", properties)
